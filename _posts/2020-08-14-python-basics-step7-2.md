...
+ __목차__
  - [1. 시작하기 전](#1-시작하기-전)
  - [2. 어떤 클래스의 인스턴스인지 확인하기: isinstance()](#2-어떤-클래스의-인스턴스인지-확인하기-isinstance)
  - [3. 특수한 이름의 메소드](#3-특수한-이름의-메소드)
  - [4. 클래스 변수와 메소드](#4-클래스-변수와-메소드)
  - [5. 가비지 컬렉터](#5-가비지-컬렉터)
  - [6. 프라이빗 변수](#6-프라이빗-변수)
  - [7. 게터와 세터](#7-게터와-세터)
  - [8. 상속](#8-상속)
...
+ 상속 : 다른 누군가가 만들어 놓은 기본 형태에서 내가 원하는 것만 교체하는 것
+ 다중 상속 : 다른 누군가가 만들어 놓은 형태들을 조합해서 내가 원하는 것을 만드는 것

+ 부모 : 기반이 되는 것
+ 자식 : 부모를 기반으로 생성한 것
+ 부모가 자식에게 자신의 기반을 물려주는 기능 => 상속
...
class Parent:
    def __init__(self):
        self.value="test"
        print("Parent 클래스의 __init()__메소드가 호출")
    def test(self):
...
## 9. 정리하기
---
+ isinstance() 함수는 어떤 클래스의 인스턴스인지 확인할 때 사용
+ 클래스 변수와 클래스 함수는 클래스 이름 뒤에 .(마침표)를 찍고 바로 사용할 수 있는 클래스가 갖고 있는 변수와 함수
