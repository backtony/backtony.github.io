...
+ 프론트쪽에서 정보를 Member로 바로 받지 않고 한 번 걸러서 받는 이유
    - 아래 코드에서는 MemberForm으로 한 번 걸러서 받았다. 화면에서 넘어오는 validation이랑 실제 domain이 원하는 validation이 다를 수 있고 여러 가지 이유로 화면에 fit한 form하나를 만들고 그걸로 데이터를 받아서 사용하는 방식을 사용하는게 좋다.
