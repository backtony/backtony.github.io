...
```
A 엔터 q엔터 를 입력하면 65 13 10 113이 출력되고 종료된다. 위 그림에서 보면 ENTER은 13과 10을 키코드로 가진다. A 엔터를 치면 버퍼에 A와 개행문자가 있는 것이다. 따라서 KeyCode 변수에는 A, 즉 65가 저장되고 이후 코드에 따라 출력된다. a는 버퍼에 남아 있는 개행 키코드에 해당하는 13을 먼저 받고 출력한다. while문에 들어와서는 버퍼에 남은 10을 받아 출력한다. 이제 버퍼가 비었으므로 다시 입력을 받는다. q를 받으면 q에 해당하는 키코드 113을 출력하고 if문에 따라 종료한다.  
<br>
...
+ 문자열 비교시 == 비교가 불가능하고 equals() 메소드를 사용해야 한다.
...
```
new는 객체생성연산자로 Scanner라는 객체를 만드는데, 괄호에는 System.in 즉, 키보드로부터 읽은 내용을 가지고 Scanner을 통문자열로 만들어준다 라고 생각하면 된다. 그 통문자를 line이라는 변수에 대입한 것이다. .nextLine() 은 Enter키 이전까지의 문자열을 읽어들인다. 
