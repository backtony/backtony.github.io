...
```
new는 객체생성연산자로 Scanner라는 객체를 만드는데, 괄호에는 System.in 즉, 키보드로부터 읽은 내용을 가지고 Scanner을 통문자열로 만들어준다 라고 생각하면 된다. 그 통문자를 line이라는 변수에 대입한 것이다. .nextLine() 은 Enter키 이전까지의 문자열을 읽어들인다. 문자열을 읽어들이므로 String 변수로 받아야 한다. 이후의 변환을 원하면 타입.parse타입() 을 이용해서 변환하면 된다. 
