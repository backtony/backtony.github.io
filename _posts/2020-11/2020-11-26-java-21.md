...
일반적인 상속의 경우 클래스 선언부에 extends를 사용하고 메인 클래스에서는 부모클래스 변수에 자식 클래스 객체를 대입하는 식으로 사용한다. 하지만 이러한 내용 없이 부모클래스 변수에 부모클래스 생성자 이후에 중괄호를 붙일 경우 이 중괄호는 클래스 선언부를 의미한다. 해석하자면 __부모클래스를 상속해서 중괄호 같은 자식 클래스 객체를 생성한 뒤에 부모클래스 변수에 대입한다__ 는 의미이다.
<br>

![그림2](https://backtony.github.io/assets/img/post/java/java-20-2.PNG)

인터페이스는 생성자를 가지고 있지 않다. 이를 해석하면 __새로운(new) 객체를 만드는데 그 객체는 중괄호같이 선언이 되는데 이름이 없다. 그것은 앞의 인터페이스를 구현해서 선언된 것이다. 이것을 가지고 객체를 만들어서 인터페이스 변수에 대입한다.__  
...
자식 객체가 부모클래스의 변수에 대입되었기 때문에 부모클래스에 있는 메소드, 필드만 사용할 수 있다. 그렇다면 부모클래스에 없고 자식클래스에 있는 필드와 메소드는 왜 선언했는가? -> 바깥에서는 사용 못하지만 내부에서 사용할 목적으로 선언한다. 즉, new Parent() { 여기서 사용 가능}
...
### 필드 선언 시 초기값으로 믹명 구현 객체 생성하여 대입하는 경우
...
### 매개 변수의 매개값으로 익명 구현 객체를 생성하여 대입
![그림7](https://backtony.github.io/assets/img/post/java/java-20-7.PNG)

메소드의 매개변수로 인터페이스 타입이 있다면, 다른 메소드에서 그 메소드를 호출할 때 매개값으로 익명 구현 객체를 대입할 수 있다.
...
+ 익명 자식 객체 : 자식 클래스가 재사용되지 않고 오로지 특정한 위치에서 사용되는 경우에 사용하면 편리
    - 부모클래스 변수 = new 부모클래스() {}
+ 익명 구현 객체 : 구현 객체 클래스가 재사용되지 않고 오로지 특정 위치에서 사용되는 경우에 사용하면 편리
    - 인터페이스 변수 = new 인터페이스() {}
