...
```
구현 클래스를 작성할 때 일일이 가져오기 불편한데 이때 선언부를 잘 확인하면 매우 간단히 해결할 수 있다.  
public class Audio implements RemoteControl {} 에서 Audio에 밑줄이 그어질 것이다.(재정의를 안했기 때문) __밑줄에 마우스를 가져다 대서 Add unimplemented methos를 클릭하면 자동으로 재정의가 필요한 추상 메소드들을 가져와 준다.__  
<br>
...
public class MyClassExam {
	public static void main(String[] args) {
		...
		// Audio는 RemoteControl 인터페이스의 구현 객체이므로 매개인자로 사용 가능
		...
		// SmartTelevision은 RemoteControl 인터페이스의 구현 객체이므로 매개인자로 사용 가능
...
+ 인터페이스 사용 : 클래스 선언 시 필드, 매개인자, 로컬 변수로 선언 가능, 구현 객체를 대입 
