...
---
+ 부모 타입을 자식 타입으로 변환
    - 조건 : __자식 타입이 부모 타입으로 자동 타입 변환된 후 다시 반대로 변환할 때 사용__
