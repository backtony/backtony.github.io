...
## 문제
---
[[클릭](https://backtony.github.io/data/2020/07/30/data-theory-step4/)]에서 구현한 연결 리스트를 기반으로 아래 구조체의 주소를 리스트에 연결한다.
```
...
__cf) 포인터를 사용하는 이유__  
 포인터를 안쓰면 계속 여러 변수명을 만들어야 합니다. 하지만 포인터를 사용하면 동적할당으로 받은 주소값을 다른 곳에 옮기면 이 포인터는 다시 동적할당으로 새로운 공간을 가리키게 할 수 있습니다. 그러므로 여러 개의 변수를 사용할 필요가 없어지는 것입니다. 따라서 노드 구조체 포인터, point 구조체 포인터를 사용한 것입니다.  
