...
루트부터 아래쪽 레벨로 노드가 가득 차 있고, 같은 레벨 안에서 왼쪽부터 오른쪽으로 노드가 채워져 있는 이진트리를 완전 이진 트리라고 한다. 높이가 k인 완전 이진 트리가 가질 수 있는 노드 수는 최대 2^(k+1) - 1이므로, n개의 노드를 저장할 수 있는 완전 이진 트리의 높이는 log n 이다.  
