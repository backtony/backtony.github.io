...
N = int(sys.stdin.readline().rstrip())
# set 함수는 입력 순서와 관계 없이 오름차순으로 자동 정렬
shop_list = set(map(int,sys.stdin.readline().rstrip().split()))

M = int(sys.stdin.readline().rstrip())
# set 함수는 입력 순서와 관계없이 오름차순으로 정렬시켜놓으므로 
# request_list는 set으로 하면 안된다.
request_list = list(map(int,sys.stdin.readline().rstrip().split()))
...
__입력 조건__  
+ 첫째 줄에 떡의 개수 N과 요청한 떡의 길이 M이 주어진다.(1<=NM=1,000,000 , 1<=M<=2,000,000,000)
