...
print(cnt)
```
모든 학생에 대해 모든 학생과의 관계를 알아야한다. 이 관계를 그래프로 표현해보자. a와 b학생의 비교 결과를 알고있다. 그래프상에서 a에서 b로, b에서 a로의 이동이 가능하다. 플로이드 워셜 알고리즘을 사용하면 이 학생을 거쳐 다른 학생으로 갈 수 있는지도 확인할 수 있다. 결과적으로 모든 학생에 대해 모든 학생과의 관계를 알 수 있게 되는 것이다. 또한, 범위가 500명까지는 O(V^3)인 복잡도까지 사용이 가능하다.
