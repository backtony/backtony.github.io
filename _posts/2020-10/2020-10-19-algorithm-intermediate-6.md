...
for _ in range(t):
    ...
    print(ans)
```
for문 안의 if문에 중복이 많은데 답압처럼 중복을 줄일 수 있다.
...
이런 누적문제를 보면 인덱스값마다 0~해당 인덱스까지의 최대값을 넣고 마지막 인덱스를 출력하면 되겠다는 생각이 든다. 그럼 이대로 설계하겠다는 생각으로 코딩을 한다. 일단 코딩을 하면서 __어떻게든 해당 인덱스에 이전까지 값들 중 최댓값을 넣을 것이라는 가정하에 코딩을 시작한다.__
```python
n = int(input())
days = [0]
pays = [0]
ans = [0] * (n + 1) # 1번 인덱스값을 수정하는데 0번 인덱스가 필요하므로
# 정보 삽입
for _ in range(n):
...
for i in range(1, n + 1):
    idx = i + days[i] - 1  # 일 한뒤에 날짜

    # 나는 설계를 시작할 때 무조건 인덱스값은 해당 인덱스까지 값들의 최댓값으로 가정했다.
    # ans[i-1]에는 i-1까지의 최댓값이 들어있을 것이고, 비교를 통해 현재위치의 최댓값을 넣는다.
    # 당일 해결하는 작업
    if days[i] == 1:
        ans[idx] = max(ans[i - 1] + pays[i], ans[idx])

    # 당일 해결 불가할 경우 현재 위치 누적값 수정
    # 위에 설명한 것과 마찬가지
    else :
        # 현재 위치에 누적값과 이전까지의 누적값 비교
        ans[i] = max(ans[i - 1], ans[i])

    # 당일 해결 불가능한 작업에 대해 퇴사 전이라면 값 수정
    if idx <= n :
        ans[idx] = max(ans[i - 1] + pays[i], ans[idx])
...
2차 리뷰 코드  
다이나믹 프로그래밍의 사고의 시작은 맨 앞에서 조금 뒤, 맨 뒤에서 조금 앞에서 점화식을 세우고 시작점을 따로 처리해주는 방식으로 사고하면 쉽게 풀린다고 했다. 전에 첫 공부때와 1차 리뷰때를 생각해보면 누적해서 끝지점에서 누적 최대값이 나와야된다고 생각했었다. 근데 지금 더 좋은 생각이 떠올랐다. 일반적인 누적의 경우 처음부터 끝으로 가면서 누적해서 마지막 인덱스의 값으로 최대값을 저장하면 쉽게 풀렸다. 이 문제의 경우 끝이 정해져있지 않다. 이걸 조금 다르게 생각해보면, 항상 시작은 1일이고 주어지는 조건에 따라 끝나는 날이 변화한다. 그럼 거꾸로 생각해서 1일이 끝나는 지점이라고 생각하면 된다. 주어짐에 따라 끝나는 날이 7일, 8일일 수도 있다. 그럼 끝나는 지점을 시작지점이라고 생각하고 1일을 끝나는 지점이라고 생각하여 진행하면 되는 것이다. 3번 문제를 풀이와 비슷하다. 3번도 결국 끝이 주어지는 입력에 따라 달라진다. 따라서 끝을 시작기준으로 잡아서 거꾸로 항상 존재하는 0번 인덱스를 끝으로 누적으로 진행했다. 똑같은 맥락이다.  
__정리하자면, 일반적인 경우 처음부터 끝까지 누적해나가면 되지만, 끝이 주어짐에 따라 변화하는 경우는 끝을 시작점으로 하여 변하지않는 시작점을 누적의 끝으로 생각하면 된다.__  
...
# 일수를 더했을 때 딱 마지막일에 걸칠 경우를 고려하여
# dp를 인덱스를 끝에 하나 더 생성해둔다.
# 끝에서부터 현재 위치가 최대수익이라 가정
...
### 내가 작성한 코드
첫 공부, 1차 리뷰에서 풀지 못했다. 다른 식으로 접근하려고는 했으나 옳지 못한 방법이었다. 증가하는 수열관련 하여 이 풀이를 꼭 기억해야겠다. 앞서 말했듯이 다이나믹 프로그래밍, 누적문제는 __점화식에 해당 가정__ 을 가지고 시작하는게 좋다고 했다. 누적문제이므로 당연히 누적을 저장할 리스트를 만드는 생각을 했고, 이 리스트의 값에 어떤 것을 넣을지 생각해야 한다. __dp[i]에는 soldier[i]를 최대값으로 하는 수열의 최대 길이가 들어있다는 가정을 잡고 시작하면 훨씬 수월하다.__
<br>
...
### 내가 작성한 코드
만약 시간이 주어졌으면 시간초과일 것이다. 아래 주석으로 작성했듯이 매번 곱으로 확인해야한다는 점이 걸리므로 곱했던 인덱스는 기억해서 다시 안하는 작업을 처리해줘야 하는데 방법이 생각나지 않는다. 사실 이런 누적문제에 코드 설계를 나처럼 하면 안됬다. 복잡도를 전혀 고려하지 않은 코드이다.  
```python
# 못생긴수끼리의 곱은 못생긴수
n = int(input())
uglys = [1]
...
while cnt != n:
    key = 0
    # ugly가 2,3,5면 추가하고 다음으로 넘어가기
    if ugly in [2, 3, 5]:
        uglys.append(ugly)
        cnt += 1
        ugly += 1
        continue
    # ugly가 uglys 리스트안의 원소들의 곱이라면 추가
    # 사실 매번 곱으로 확인해야한다는 점이 복잡도가 너무 크다.
    # 곱했던 인덱스는 기억하는 장치가 필요해
    for i in range(1,len(uglys)):
        for j in range(i,len(uglys)):
            if uglys[i] * uglys[j] == ugly:
                key = 1
                break
        if key:
            uglys.append(ugly)
...
2차 리뷰 코드  
못생긴 수는 못생긴 수에 2,3,5만을 곱해서 만들면 된다. 각 못생긴 수 마다 2,3,5를 곱할 껀데, 2,3,5를 곱했던 인덱스 위치를 따로 기억해두어 중복을 막고 곱한 결과값끼리 비교해서 최소값을 ugly 배열에 넣는다.
```python
n = int(input())
ugly = [1]
...
# 같은 경우는 중복해서 ugly 배열에 넣으면 안되므로 if문으로만 작성
for i in range(1, n):
    temp = min(next5, next3, next2)
    ugly.append(temp)
    if temp == next5:
        idx5 += 1
        next5 = ugly[idx5] * 5
...
시작하기 전에 혼동하지 말자. su와 saturday를 비교할 때 su와 sa비교 이후에는 계속 u와 turday를 비교하는 것이다.  
+ 삽입 : 표 왼쪽값 +1
...
+ 삭제 : 위쪽의 값 +1
    - 표에는 없지만 이해하기 쉽게 su과 sun를 비교해보자. 빈문자와 n를 비교하게 되어 다른 문자로 판단된다. 그럼 su와 su를 비교하여 저장했던 값에서 sun의 n을 빼는 과정 +1을 해주는 것이다. 결과적으로 sun을 su로 만들어 맞출수 있게 된다.
+ 비교했을 때 같은 문자로 판단되면, su 와 su를 비교하면 u는 같은 문자로 판단되어 s와 s끼리 비교하여 저장한 값을 그대로 가져온다. 같은 문자의 경우 위 3가지 경우를 처리할 필요가 없기 때문이다.

위와 같은 과정을 통해 비교해서 편집 거리가 가장 작은 값을 표의 각 위치에 저장하면 된다. 결국 표의 제일 마지막 인덱스를 출력하면 최소 편집 거리가 출력된다.  
