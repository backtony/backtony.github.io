...
이런 누적문제를 보면 인덱스값마다 0~해당 인덱스까지의 최대값을 넣고 마지막 인덱스를 출력하면 되겠다는 생각이 든다. 그럼 이대로 설계하겠다는 생각으로 코딩을 한다. 일단 코딩을 하면서 __나는 어떻게든 해당 인덱스에 이전까지 값들 중 최댓값을 넣을 것이라는 가정하에 코딩을 시작한다.__
```python
n = int(input())
days = [0]
pays = [0]
