---
layout: post
title:  API 실습1 - 모두의책리뷰
subtitle:   API 실습1 - 모두의책리뷰
categories: web
...
## 1. 문제
---
[완성 예시 클릭](http://spartacodingclub.shop/bookreview){: target="_blank"}  
주어진 코드를 가지고 완성 예시처럼 리뷰 작성하기 버튼을 클릭하면 아래 박스에 기록되는 페이지를 만들어보자.  
...
## 2. 실습
---
1. 클라이언트는 데이터를 서버로 전달
2. 서버는 DB에 데이터 저장 (POST 타입 API)
3. 클라이언트가 데이터 요청
4. 서버에서 DB 정보 반환 (GET 타입 API)
5. jQuery를 이용해 붙이기
