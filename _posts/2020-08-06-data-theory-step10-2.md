...
```
### 성능평가
힙의 빅-오는 O($$\log_2 n$$) 이고 n개의 데이터를 뽑아내므로 O(n$$\log_2 n$$) 이다.  
<br>
...
정렬의 대상인 데이터의 수가 n개일 때, 각 병합의 단계마다 최대 n번의 비교연산이 진행된다. 데이터의 수 n과 그에 따른 병합 과정의 횟수는 $$\log_2 n$$이다. 따라서 데이터 수에 대한 비교 연산의 횟수는 n$$\log_2 n$$이고 빅-오는 O(n$$\log_2 n$$) 이다.  
<br>
...
모든 데이터가 피벗과의 데이터 비교를 진행한다. 따라서 이 경우 약 n번의 비교 연산이 진행된다. 최선의 경우 $$\log_2 n$$번 이루어 지므로 빅-오는 O(n$$\log_2 n$$)이다.
