...
## 2. 힙 정렬
---
힙 정렬은 힙의 특성을 이용하여 정렬하는 알고리즘이다. 힙은 '부모의 값이 자식의 값보다 항상 크다'는 조건을 만족하는 완전 이진 트리이다. 이때 부모의 값이 자식의 값보다 항상 작아도 힙이라고 한다. 즉, 이러한 두 값의 대소 관계가 일정하면 된다.  
...
def heap_sort(a):
    ...
    def down_heap(a,left,right): 
        ...
        while left < (right+1)//2: # 마지막 부모 노드 +1
    ...
    # 힙 만들기
    # (n-1)//2 # 마지막 부모 노드, 짝수의 경우 마지막 부모노드 +1 인데 while문에서 걸러짐
    # 마지막 부모 노드부터 힙으로 만들면서 위로 올라감
    for i in range((n-1)//2,0-1,-1):
        down_heap(a,i,n-1)
    
    # 정렬하기
    for i in range(n-1,0,-1):
        a[0],a[i] = a[i],a[0]
        # a[i]는 정렬완료, 이외의 원소중 a[0]을 제외한 것은 모두 힙 상태
        down_heap(a,0,i-1)
...
for i in range(n):
        a[i]=b[i]
대신 a=b는 사용할 수 없다.
b[i]는 각각 int형 객체를 참조하고 있고 이 참조값을 a[i]에 전달한다.
하지만 a=b의 경우 b는 리스트 객체를 참조하고 이 객체는 함수 내에서 정의되었다.
따라서 외부에서 b의 참조값을 사용할 수 없게된다.
```
