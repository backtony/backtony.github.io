...
### 내가 작성한 코드
첫 공부할 때는 풀지 못했고, 1차 복습에서도 한 번에 풀지 못했다.  
방향 변화에서 코드가 너무 길어져서 답안과 비교해보니 답안은 좌표를 이용해서 방향을 처리했다. 그 방법이 더 좋은 것 같다. 또한 실제로 문제가 주어졌을 때 회전의 이동변환을 for문으로 한 번에 처리하는 것을 생각해 낼 수 있을지 의문이다. 처음 코딩했을 때는 규칙성 없이 x1,y1,x2,y2를 마구 사용하다 보니 어떤 경우에는 맞고 어떤 경우에는 맞지 않았다. 다시 생각해보면 __회전할 때 고정점을 기준으로 좌표를 처리하면 규칙성 있게 처리할 수 있다. 고정점을 기준으로 하면 앞뒤 좌표가 바뀌어도 결국에는 모든 경우를 확인할 수 있다.__
```python
from collections import deque
...
# 주어진 position에서 이동 가능한 위치 찾기
def possible_way(board, way, position):
    possible = []  # 이동 가능 위치 저장소
    position = list(position)  # 언팩을 위해 리스트 형변환

    x1, y1, x2, y2 = position[0][0], position[0][1], position[1][0], position[1][1]
    ...
    for i in range(4):
        ...
        if board[px1][py1] == 0 and board[px2][py2] == 0:
            possible.append((way,{(px1, py1), (px2, py2)}))

    # 회전
    # 가로방향
    # 움직이지 않는 점을 기준으로 좌표 작성해야 좌표 앞뒤가 바뀌어도 올바르게 사용가능
    if way == 0:
        # 아래쪽 빌때, 위쪽 빌때
        for i in [1, -1]:
            if board[x1 + i][y1] == 0 and board[x2 + i][y2] == 0:
                # 방향이 변함
                possible.append((1, {(x1, y1), (x1 + i, y1)}))
                possible.append((1, {(x2, y2), (x2 + i, y2)}))
    ...
    # 세로 방향
        ...
        for i in [1, -1]:
            if board[x1][y1 + i] == 0 and board[x2][y2 + i] == 0:
                # 방향이 변함
                possible.append((0, {(x1, y1), (x1, y1 + i)}))
                possible.append((0, {(x2, y2), (x2, y2 + i)}))
...
def solution(board):
    ...
    q = deque()
    # 시간, 방향(가로0,세로1), 현재 위치 삽입
    q.append((0, 0, {(1, 1), (1, 2)}))
    visited.append({(1, 1), (1, 2)})  # 방문 처리

    # bfs
    while q:
        cnt, way, position = q.popleft()
        # 목적지 도착,, 목적지가 항상 도착할 수 있도록 보드가 주어짐
        if (n, n) in position:
            return cnt
        for new_way, pos in possible_way(new_board, way, position):
            # 방문한적 없으면
            if pos not in visited:
                # 큐 삽입, 방문처리
                q.append((cnt + 1, new_way, pos))
                visited.append(pos)
```
<Br>

2차 리뷰 코드  
__bfs의 풀이의 경우, 큐에서 좌표가 처음 꺼내지는 그 순간이 최소거리, 최소도착시간이다.__ 따라서, 큐에 시간과 좌표를 넣고 bfs를 하면 된다.
```python
from collections import deque


# 가능한 이동경로와 시간 반환
def check(pos, new_board):
    possible = []
    dx = [-1, 0, 1, 0]
    dy = [0, 1, 0, -1]
    pos = list(pos)
    ...
    # 세로 가로인지에 따라 회전 경우 나누기
    # 가로인경우의 회전
    if x1 == x2:
        # 위, 아래 회전
        for i in [-1, 1]:
            if new_board[x1 + i][y1] == 0 and new_board[x2 + i][y2] == 0:  # 회전이 가능한지 확인
                possible.append({(x1, y1), (x1 + i, y1)})
...
def solution(board):
    n = len(board)
    # 4방향과 회전 2번 나눌 때 모두 범위내인지 확인하기 번거로우니까
    # 새로운 보드 만들어서 1로 둘러쌓고 범위내 구분하지 말고 1로만 구분
    new_board = [[1] * (n + 2) for _ in range(n + 2)]
    for i in range(n):
        for j in range(n):
            new_board[i + 1][j + 1] = board[i][j]

    q = deque([(0, {(1, 1), (1, 2)})])  # 시간, 첫시작위치
    visited = [{(1, 1), (1, 2)}]  # 방문 처리
    ...
    while q:
        ...
        possible_way = check(pos, new_board)  # 가능한 위치 받고
        for possible in possible_way:
            if possible not in visited:  # 방문한적이 없다면
