...
### 내가 생각한 풀이
보텀업 방식으로 접근을 해보았는데, 테이블을 만들어서 false로 초기화하고 뽑힌 화폐는 true, 이전에 만들 수 있는 금액을 찾아 거기에 뽑힌 화폐를 더한 금액에 true로 수정하는 과정을 반복하여 최종적으로 배열을 맨 앞부터 훑어보면서 false가 나오는 수를 출력하는 알고리즘을 설계했으나 __화폐 단위가 최대 100만이기 때문에 누적으로 처리해나가기에는 무리가 있다.__
...
__답안처럼 while문전에 pop으로 빼지않고 while 조건문으로 우선순위 큐를 이용하고 조건에 통과하면 빼는 방식을 사용하는 것이 좀 더 간단해 보인다.__ 1차랑 똑같은 점을 느낀다.
```python
import heapq
...
__cf 2차 리뷰 결과 -> 5번 7번 다시 복습할 것__
