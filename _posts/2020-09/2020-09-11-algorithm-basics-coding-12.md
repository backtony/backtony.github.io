...
### BFS
BFS는 너비 우선 탐색이라고 하며, 가까운 노드부터 탐색하는 알고리즘이다. BFS의 구현에서는 큐 자료구조를 이용하는 것이 정석이다. DFS와 마찬가지로 방문 리스트를 따로 작성한거나 주어진 그래프에 덮어씌우면서 방문표시를 한다는 점을 기억하자.  
...
__대표적인 예제__  
+ 여러가지 연산이 주어지고 원하는 수를 만드는 최소 연산 횟수  
...
2. 출발 노드 우선순위 큐에 삽입과 __최단 거리 테이블의 인덱스 출발 노드를 0으로 초기화__
3. while문을 통해 q가 빌 때까지 4 ~ 5 과정을 반복
4. 큐에서 꺼내고 해당 노드를 거쳐 방문하지 않은 연결되있는 노드로 가는 비용을 계산하여 최단 거리 테이블 갱신
5. 방문하지 않은 연결된 노드를 큐에 삽입
...
이거는 내 생각인데 위상 정렬의 대표적인 문제 커리큘럼은 선수과목이 없는 즉, 진입차수가 0인 과목(처음부터)부터 시간을 누적으로 계산해야 한다는 점에서 다이나믹 프로그래밍과 유사한 점이 있는 것 같다.  
