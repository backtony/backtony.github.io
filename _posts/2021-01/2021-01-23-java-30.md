...
## 4. Map 컬렉션
---
Map 컬렉션은 key와 value로 구성된 Map.Entry 객체를 저장하는 구조를 가지고 있다. Entry는 Map 인터페이스 내부에 선언된 중첩 인터페이스이다. key와 value는 모두 객체이다. key값은 중복될 수 없고, value값은 중복이 가능하다. 만일 동일한 key로 저장될 경우 새로운 값으로 대체된다.  
