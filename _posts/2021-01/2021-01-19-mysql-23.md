...
```
브라우저에서 파일이 있는 위치를 입력하면 확인이 가능하다. 나의 경우 localhost/13-01.php로 확인 가능했다. 이 화면이 출력되려면 xampp에서 xampp-control.exe 로 apache 실행해야 한다.
...
<?php
   ...
   while($row = mysqli_fetch_array($ret)) {
     ...
     // get방식으로 날린다. ?변수=값 형식
     // html과 php문을 구분해야하기에 문자열 끊어서 사용
