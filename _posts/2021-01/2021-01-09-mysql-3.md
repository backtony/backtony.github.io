...
+ __목차__
  - [1. 외부 MySQL 서버 관리(Linux 서버)](#1-외부-mysql-서버-관리-linux-서버)    
  - [2. 사용자 관리](#2-사용자-관리)    
...
Administrative Roles 탭에서 맨위에 DBA를 체크해주게 되면 사실상 모든 권한이 부여되게 되고 Apply를 누르면 적용된다. 이렇게 데이터베이스 관리작 팀장님의 사용자 권한을 부여했다.
...
이제 첫 화면에서 오른쪽 클륵 ->  Edit connection -> 사용자이름 변경 한 뒤에 접속하면 해당 사용자로 접속할 수 있게 된다.  

__cf) 명령어 모드에서 사용자 생성 삭제하기__  
```
CREATE USER 사용자이름@'%'IDENTIFIED BY '비밀번호';
GRANT SLELECT,INSERT,UPDATE,DLELTE ON shopdb.*TO 사용자이름@'%'; // shopdb 데이터베이스에 다음 권한을 이름에게 부여
