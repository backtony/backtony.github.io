---
layout: post
title:  파이썬 파트 8-2. 외부 모듈 및 만들기
subtitle:   파이썬 파트 8-2. 외부 모듈 및 만들기
categories: python
...
+ __목차__
  - [1. 모듈 설치하기](#1-모듈-설치하기)
  - [2. BeautifulSoup 모듈](#2-BeautifulSoup-모듈)
  - [3. Flask 모듈](#3-Flask-모듈)
  - [4. 라이브러리와 프레임워크](#4-라이브러리와-프레임워크)
  - [5. 함수 데코레이터](#5-함수-데코레이터)
  - [6. 모듈 만들기](#6-모듈-만들기)
  - [7. \_\_name\_\_ == "\_\_main\_\_"](#7-name--main)
  - [8. 패키지](#8-패키지)
  - [9. 텍스트 데이터와 바이너리 데이터](#9-텍스트-데이터와-바이너리-데이터)
  - [10. 정리하기](#10-정리하기)
...
print(module_a.input_a) # module_a.py에 input_a 변수가 있다고 가정하고 작성
print(module_b.input_b) # module_b.py에 input_b 변수가 있다고 가정하고 작성
```
<br>
...
### 텍스트 데이터
우리가 쉽게 읽을 수 있는 형태의 데이터를 '텍스트 데이터'라고 부른다. 텍스트 데이터는 쉽게 읽을 수 있고, 텍스트 에디터만 있으면 쉽게 편집할 수 있다. 우리가 입력해 왔던 모든 코드 역시 텍스트 데이터이다.  

### 바이너리 데이터
텍스트 에디터에 '100'이라는 숫자를 표현하는 경우 100이라는 글자는 '1' '0' '0'이라는 글자로 이루어져 있으며 아스키 코드표를 살펴보면 '49' '48' '48'로 변환할 수 있다는 것을 알 수 있다. 따라서 저장되면 3바이트를 차지하는 텍스트 데이터로 00110001 00110000 00110000 으로 저장된다. 그런데 100을 '1' '0' '0'으로 나타내는 텍스트 데이터가 아니라, 곧바로 '100'이라는 숫자로 저장한다면 한 글자로 표현할 수 있게 되므로 용량이 절약될 것이다. 그렇다면 100은 1바이트를 차지하는 바이너리 데이터 01100100 으로 표현될 수 있다. 이 100이라는 숫자는 아스키 코드표에서 살펴보면 'd'에 해당한다. 따라서 텍스트 에디터로 이러한 파일을 읽어 들이면 'd'라는 내용이 들어있을 것이다. 즉, 텍스트 데이터로 표현할 경우 의미를 알 수 없는 데이터가 되버리는 것이다. => 이처럼 텍스트 에디터로 열었을 때 의미를 이해할 수 없는 데이터를 바이너리 데이터라고 부른다. 바이너리 데이터의 대표적인 예는 이미지와 동영상이다.  
<br>

## 10. 정리하기
---
+ 외부 모듈은 파이썬이 기본적으로 제공하지 않는, 다른 사람들이 만들어 제공하는 모듈을 의미
+ pip install 은 외부 모듈을 설치할 때 사용하는 명령어
+ 제어 역전(Inverse of Control)은 개발자가 모듈의 함수를 호출하는 것이 일반적인 제어 흐름이나, 이와 반대로 개발자가 만든 함수를 모듈이 실행하는 것을 의미
+ 라이브러리는 개발자가 모듈의 기능을 호출하는 형태와 같이 정상적인 제어를 하는 모듈
+ 프레임워크는 모듈이 개발자가 작성한 코드를 실행하는 형태의 모듈
+ 엔트리 포인트는 python 명령어를 사용해서 진입하게 되는 첫 진입 파일
+ \_\_name\_\_ == "\_\_main\_\_"는 현재 파일이 엔트리 포인트인지 아닌지 확일할 때 사용하는 코드
+ 패키지는 모듈이 모인 것
