...
void SInsert(List * plist, LData data)
{
  ...
  //다음이 끝이 아닐때와 비교과정에서 받은 데이터값이 기존 값보다 head에서 더 멀리 떨어져야 할때
	{
		pred = pred->next;
	}
...
LData LRemove(List * plist)
{
  ...
  연결 리스트의 삭제는 참조과정에서 삭제를 진행하므로 참조 과정에서 자동적으로 before는 cur을 받고
  cur은 앞 노드로 이동기 때문에 before은 신경쓰지 않아도 된다. */
